...
## Monitor Conditions Overview

Monitor conditions define when a monitor should trigger. Conditions compare the calculated metric value against thresholds using comparison operators.
...
All child conditions must be true for the monitor to trigger.

**Example**: 
```
(ForwardPercentage >= 20) AND (TotalInteractions > 10)
```
Monitor triggers when escalation rate is high AND there are many interactions

### OR

At least one child condition must be true for the monitor to trigger.
...
Duration > 600000
```
Monitor triggers when average duration exceeds 10 minutes
...
```
Monitor triggers when there are many interactions AND high escalation rate
...
```
Monitor triggers when either type of failure is high
...
```
Monitor triggers when there are many interactions AND (high escalation OR long duration)
...
- **Monitor and adjust**: Review monitor history and adjust thresholds based on actual patterns
...
1. **At configured intervals** — Based on monitor's `Interval` setting (default: 60 seconds)
