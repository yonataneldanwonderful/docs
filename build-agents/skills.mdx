...
<Steps>
  ...
  <Step title="Attach skills to an agent">
    ...
    3. Click **+ Add Skill** and select the skills you want to attach
    4. Confirm each attached skill is Active and correctly configured
  </Step>

  <Step title="Choose a loading mode">
    On the agent's **Settings tab**:
    - **Default mode**: All attached skills are loaded from the start—best for simple agents with a small number of related skills
    - **Multi-skill mode**: Toggle on **Multi-skill** to have the agent load one skill at a time and switch as needed—best for agents spanning multiple distinct workflows
  </Step>
